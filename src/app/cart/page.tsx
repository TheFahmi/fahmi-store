...
export default function CartPage() {
  ...
  if (!user) {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <p className="mt-2 text-muted-foreground text-sm">Sign in to view your cart</p>
        <Link
          href="/login"
          className="inline-flex mt-6 h-11 px-6 items-center rounded-full bg-primary text-primary-foreground text-sm font-medium"
        >
          Sign in
        </Link>
      </div>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
      <h1 className="text-3xl font-semibold tracking-tight text-foreground">Cart</h1>
      ...
      {loading ? (
        ...
        <div className="mt-16 text-center">
          ...
          <Link href="/products" className="inline-flex mt-4 text-sm text-primary hover:underline">
            Browse products
          </Link>
        </div>
      ) : (
        <>
            ...
            {items.map((item) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">{formatIDR(Number(item.harga))}</p>
          ...
          <div className="mt-8 p-5 rounded-2xl bg-card border border-border">
            ...
            <button className="mt-4 w-full h-11 rounded-full bg-primary text-primary-foreground text-sm font-medium hover:opacity-90">
              Checkout
            </button>
            <p className="mt-2 text-xs text-center text-muted-foreground">Checkout flow uses existing backend order APIs</p>
          </div>
        </>
      )}
    </div>
